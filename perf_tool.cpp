...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < ehdr->e_shnum; ++i) {
        if (shdrs[i].sh_type == SHT_SYMTAB) {
            ...
            cout << "sh_link is " << shdrs[i].sh_link << std::endl;
            // shdrs[i].sh_link = 2;
            // const char *symtab_str = (const char *)mem +
            ...
            for (size_t j = 0; j < shdrs[i].sh_size / sizeof(Elf64_Sym); ++j) {
                if (ELF64_ST_BIND(syms[j].st_info) == STB_GLOBAL && first_global == -1) {
                    first_global = j;
                }
                if (ELF64_ST_TYPE(syms[j].st_info) == STT_FUNC) {
                    // ark text start offset is 0x1000;
                    cout << "st_value is " << syms[j].st_value << endl;
                    syms[j].st_value = syms[j].st_value + 0x1000;
                    cout << "After add 1000 st_value is " << syms[j].st_value << endl;

                    // section 1 is ark aot .text
                    cout << "shndx is " << syms[j].st_shndx << std::endl;
                    // syms[j].st_shndx = 1;
                }
                ...
                /* std::cout << symtab_str + syms[j].st_name << "\t value:" <<
                 * syms[j].st_value << " \t shn:" << syms[j].st_shndx << std::endl; */
            }
            cout << "sh_info is " << shdrs[i].sh_info << std::endl;
            cout << "first_global is " << first_global << std::endl;
